...
module ModeUartNotifier #(
    ...
    // 【关键修改】：加入 (mode_state != mode_state_d) 
    // 使得 mode 改变的瞬间 busy 立即变高，防止其他模块在 pending_req 锁存前误判为空闲
    assign busy = (state != ST_IDLE) || pending_req || in_reset_guard || (mode_state != mode_state_d);
